...
public:
    CGame()
        : creature(1, 3, maze), monster(2, 10), foodCount(0) {
        maze.readMaze();
        maze.drawMaze();
...
void CGame::gameLoop() {
    ...
    while (restart) {
        ...
        move = 'y';
        creature.setPosition(1, 3);
        monster.setPosition(2, 10);
        foodCount = 0;

        maze.readMaze();
        // Count initial food count
...
void CGame::moveMonster() {
    ...
    creature.getPosition(playerRow, playerCol);
    monster.getPosition(monsterRow, monsterCol);

    int newRow, newCol;
    if (playerRow < monsterRow && creature.isValidMove(monsterRow - 1, monsterCol)) {
        newRow = monsterRow - 1; // Move up
        newCol = monsterCol;
    }
    else if (playerRow > monsterRow && creature.isValidMove(monsterRow + 1, monsterCol)) {
        newRow = monsterRow + 1; // Move down
        newCol = monsterCol;
    }
    else if (playerCol < monsterCol && creature.isValidMove(monsterRow, monsterCol - 1)) {
        newRow = monsterRow;
        ...
        // The player is either on the same row or column as the monster, but there is a wall blocking its movement in the direction of the player
        // In this case, the monster can randomly choose to move in any available direction
        ...
        while (numDirections > 0 && !foundValidMove) {
            ...
            if (creature.isValidMove(monsterRow + dx, monsterCol + dy)) {
